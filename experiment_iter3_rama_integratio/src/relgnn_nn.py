...
class RelGNN(torch.nn.Module):
    def __init__(
        ...
        # When aggr is a custom Aggregation object (e.g. RAMA), use it for
        # SAGEConv inside RelGNNConv but keep "sum" for cross-route grouping
        # in RelGNN_HeteroConv.group() which requires a string.
        group_aggr = aggr if isinstance(aggr, str) else "sum"
